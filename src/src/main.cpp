...
const string strMessageMagic = "Gxcoin Signed Message:\n";
...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uGXC
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    if (fAllowFree)
    {
        if (nBlockSize == 1)
        {
            ...
            // (about 4500 GXC if made of 50 GXC inputs)
            if (nBytes < 10000)
                nMinFee = 0;
        }
        else
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GxcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GxcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GxcoinMiner(CWallet *pwallet)
{
    printf("GxcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gxcoin-miner");
    ...
    try { loop {
        ...
        printf("Running GxcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GxcoinMiner terminated\n");
        throw;
    }
}

void GenerateGxcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GxcoinMiner, pwallet));
}
