...
{
    QString ret = QString("gxcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non GXC unit input in lnReqAmount, we generate the URI with GXC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(GxcoinUnits::format(GxcoinUnits::GXC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
